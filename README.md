...
```sh
open /Users/ufukkeskin/portfolio/index.html
```

Or open `index.html` directly in your browser. All four tabs and the local fonts work from disk.
...
| File | Contents |
| --- | --- |
| `index.html` | About Me / landing page, contact details, full-width campus panorama |
| `project.html` | Project overview and latest research update |
| `sofar.html` | Chronological steps and milestones |
| `blog.html` | Informal posts; a commented article template is included |
| `style.css` | Shared fonts, blue navigation, page layout, responsive styles |
| `assets/` | Original vector illustrations, 6K PNG download, local fonts |
...
Replace the “Research updates to come” note in `project.html` with a **dated, factual** description of the latest work. Link relevant code, notes, or papers. Keep the newest state here and move older steps into `sofar.html`.

### Add a progress entry

Copy a `<li>...</li>` inside the `.timeline` list in `sofar.html`. Give it a title, your actual date or step number, a short explanation, and relevant links. The current entries only describe the initial project direction and portfolio setup; they do not claim completed research experiments.

### Add a blog post

In `blog.html`, remove the visible “No posts yet” section. Copy the commented `<article>` template out of its comment, then replace its title, `id`, date, and text. Put the newest post first. Use a unique `id` for each post so its title link works as a permalink. The sample date inside the comment is not a published entry.
...
cd /Users/ufukkeskin/portfolio
git status
git add index.html project.html sofar.html blog.html style.css assets tools README.md .gitignore .nojekyll
git commit -m "Revise portfolio layout and campus artwork"
...
On GitHub, use **Settings → Pages → Deploy from a branch → main → /(root)**. After the Pages deployment completes, visit the live URL and hard-refresh with **Command-Shift-R** if an old stylesheet is cached. `.nojekyll` tells Pages to serve the files without Jekyll processing.
...
See [assets/README.md](assets/README.md) for illustration limitations, visual references, credits, font licensing, and replacement guidance.

### Campus breeze animation

Five small leaves drift and turn across the home-page illustration on staggered 22–31 second loops. They are a separate, decorative SVG layer in `index.html`, animated by CSS in `style.css`. The buildings, sculpture, original SVG/PNG downloads, and easter egg are unchanged.
...
- To tune the effect, edit the `.breeze-leaf` timing and drift variables in `style.css`.
...
They validate local links and fragments, navigation consistency, image references, SVG parsing, font assets, and the 6K PNG dimensions.
